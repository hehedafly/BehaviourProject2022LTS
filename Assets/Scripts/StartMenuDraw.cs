...
class ConfigBody{
    ...
    public int INIWriteContent(Ini_reader ini_reader){//写入ini的键名称和代码中变量名不同
...
class Config{
    ...
    public int INIWriteAll(Ini_reader ini_reader){
...
public class StartMenuDraw : MonoBehaviour
{
    ...
    {   
        tableDraw = Rows.GetComponent<TableDraw>();
            ...
            ini_reader = new Ini_reader(context_config_path+metaConfigName);
